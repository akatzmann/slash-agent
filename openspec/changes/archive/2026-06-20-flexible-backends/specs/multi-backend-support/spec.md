...
The agent system SHALL configure the chosen backend using the environment variables `AGENT_ENDPOINT`, `AGENT_MODEL`, and other backend-specific variables (like `OPENAI_API_KEY`). If no endpoint or model is specified:
- For `openai`: `AGENT_MODEL` defaults to `gpt-4o-mini`, and the endpoint defaults to OpenAI's official endpoint.
- For `ollama`: `AGENT_ENDPOINT` defaults to `http://127.0.0.1:11434`, and `AGENT_MODEL` defaults to `gemma4:e4b-it-qat`.
...
- **THEN** the agent defaults to using the `gpt-4o-mini` model with OpenAI standard client configuration.
