...
* **Background Task Tools:** Introduce new tools (`list_background_tasks`, `get_task_logs`, `kill_background_task`) to allow the model to manage services interactively.
...
- `shell-agent`: Add background task tracking, log querying, task termination tools, and orphan-proof cleanup handlers.

## Impact

* **`slash_agent/tools.py`:** Create task execution wrappers and cleanup handles; add list, logs, and kill tools.
* **`slash_agent/main.py`:** Register new task-management tools; update system prompts and setup process group flags.
