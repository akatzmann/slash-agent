...
The `write_file` tool SHALL reject any path that is not absolute. If a relative path is supplied, the tool SHALL return an error string describing the requirement without writing any file.

#### Scenario: Relative path rejected
- **WHEN** the agent calls `write_file` with a path that does not begin with `/`
- **THEN** the tool returns an error message stating that an absolute path is required and no file is written
